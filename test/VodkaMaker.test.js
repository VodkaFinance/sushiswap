...
const VodkaMaker = artifacts.require('VodkaMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('VodkaMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.vodka = await VodkaToken.new({ from: alice });
        await this.vodka.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await VodkaMaker.new(this.factory.address, bar, this.vodka.address, this.weth.address);
        this.vodkaWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.vodka.address)).logs[0].args.pair);
    ...
    it('should make VODKAs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.vodkaWETH.address, '10000000', { from: minter });
        await this.vodka.transfer(this.vodkaWETH.address, '10000000', { from: minter });
        await this.vodkaWETH.mint(minter);
        ...
        // After calling convert, bar should have VODKA value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.vodka.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for VODKA-ETH pair
        await this.vodka.transfer(this.vodkaWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.vodkaWETH.address, '100000', { from: minter });
        await this.vodkaWETH.sync();
        await this.vodka.transfer(this.vodkaWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.vodkaWETH.address, '10000000', { from: minter });
        await this.vodkaWETH.mint(minter);
        assert.equal((await this.vodkaWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.vodka.address, this.weth.address);
        assert.equal((await this.vodka.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.vodkaWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});